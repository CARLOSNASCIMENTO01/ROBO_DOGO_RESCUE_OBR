# over_rasp_webcam.py
# Adaptado por: ChatGPT (base: over_rasp.txt)
# Mudanças principais:
# - Picamera2 -> OpenCV VideoCapture (webcam USB)
# - Removido YOLO silver (detecção da fita prateada)
# - Removida lógica de saída de zona
# - Removed shared_memory image buffer (não mais usado)
# - Adicionado controle direto dos motores L298N (canal A = esquerda, canal B = direita)
# - Se RPi.GPIO não disponível usa DummyGPIO para debug em PC
...
# Importa seus gerenciadores existentes (Timer, mp_manager, config_manager, e variáveis)
# mantive isso para compatibilidade com o resto do projeto.
from Managers import Timer
from mp_manager import *  # espera-se que este arquivo crie/defina variáveis como line_angle, line_detected, etc.

# -------- GPIO / Motor setup (L298N) ----------
try:
    import RPi.GPIO as GPIO
    ON_RPI = True
except Exception:
    ON_RPI = False
    # Dummy GPIO para debug em PC
    class DummyGPIO:
        BCM = 'BCM'
        OUT = 'OUT'
        HIGH = 1
...
# Default GPIO mapping (BCM) - pode trocar depois
IN1 = 17  # Direção motor A
IN2 = 27
ENA = 18  # PWM motor A

IN3 = 22  # Direção motor B (corrigido: estava faltando no código)
IN4 = 23
ENB = 13  # PWM motor B
...
pwm_left = GPIO.PWM(ENA, 5000)  
pwm_right = GPIO.PWM(ENB, 5000)
pwm_left.start(0)
pwm_right.start(0)
...
def set_motors_direction(left_forward: bool, right_forward: bool):
    """Define as direções do motor (True = frente, False = ré) por canal."""
    # Esquerda
    if left_forward:
...
def set_motor_speed(line_angle: float, line_detected_bool: bool, base_speed: float = 60):
    """
    Controla os dois canais (A = esquerda, B = direita) em PWM.
    line_angle: -180..180 (neg = left, pos = right)
    line_detected_bool: se a linha está detectada (senão - parar)
    base_speed: % dutycycle base (0-100)
    """
    # Se a linha não está detectada, para (ou aplica uma busca).
    if not line_detected_bool:
        motors_stop()
        return

    # Normaliza line_angle e calcula correção proporcional
    # Assumimos que +/-90 dá correção máxima.
    correction = np.clip(line_angle / 90.0 * 40.0, -40.0, 40.0)  # ajuste de ganho; pode calibrar
    left_speed = np.clip(base_speed - correction, 0, 100)
    right_speed = np.clip(base_speed + correction, 0, 100)

    # Definir direção (assumindo que velocidades sempre positivas = frente)
    set_motors_direction(True, True)

    pwm_left.ChangeDutyCycle(float(left_speed))
    pwm_right.ChangeDutyCycle(float(right_speed))


# -------- Camera / image settings ----------
# O usuário pediu 640x640 caso não especificado
camera_x = 640
camera_y = 640

calibration_square_size = 25

# Color thresholds (mantive os valores / leitura via config_manager)
...
# ---- funções utilitárias do teu arquivo (mantidas) ----
...
def update_color_values():
    ...
    # as mesmas leituras do config_manager (supondo que config_manager exista)
    try:
        ...
        # se config_manager não existir no contexto (debug), usa os valores default definidos acima
        pass
...
########################################################################################################################
# Line Cam Loop (principal) - usa webcam USB via OpenCV
########################################################################################################################

def line_cam_loop():
    global cv2_img, x_last, y_last, time_line_angle, multiple_bottom_side

    # Removi o modelo YOLO / silver (não será usado)
    ...
    # Algumas webcams precisam setar o FOURCC ou fps, normalmente ok sem isso

    # Se não conseguiu abrir, tenta índice 1
    if not cap.isOpened():
        cap.open(1)
    if not cap.isOpened():
        raise RuntimeError("Não foi possível abrir a webcam (VideoCapture). Verifique a conexão USB.")
    ...
    black_min = black_min.astype(np.uint8)
    black_max_normal_bottom = np.clip(black_max_normal_bottom, 0, 255).astype(np.uint8)
    black_max_normal_top = np.clip(black_max_normal_top, 0, 255).astype(np.uint8)
    black_max_ramp_down_top = np.clip(black_max_ramp_down_top, 0, 255).astype(np.uint8)

    # Main loop
    try:
        while not terminate.value:
            ret, raw_capture = cap.read()
            if not ret:
                # tenta recuperar a câmera
                cap.release()
                cap = cv2.VideoCapture(0)
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, camera_x)
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, camera_y)
                time.sleep(0.1)
                continue

            # Redimensiona para as dimensões definidas (garante formato esperado)
            raw_capture = cv2.resize(raw_capture, (camera_x, camera_y))
            cv2_img = raw_capture.copy()

            # Se o objetivo for "zone" e certos estados -> menor frame_limit
            ...
            if time.perf_counter() - fps_limit_time > 1 / frame_limit:
                ...
                if calibrate_color_status.value == "none":
                    ...
                    # ***** SILVER REMOVIDO *****
                    # Removi o bloco que fazia inferência com o modelo YOLO para detectar prata.

                    if objective.value == "follow_line":
                        # Processamento HSV e detecção de cores
                        ...
                        # Ajusta a calibração do preto para entradas de zona (mantive a lógica)
                        ...
                        # (A lógica relacionada a silver foi removida - silver_image, calc_silver_angle etc.)
                        ...
                        blk_contour_area = np.array([cv2.contourArea(i) for i in contours_blk]) if len(contours_blk) > 0 else np.array([])
                        blk_mask = blk_contour_area > min_line_size.value if blk_contour_area.size > 0 else np.array([])
                        contours_blk = [c for c, m in zip(contours_blk, blk_mask) if m] if blk_mask.size > 0 else []
...
multiple_bottom_side = camera_x / 2
cap = cv2.VideoCapture(0)
# ========== SIMULAÇÃO DO CONFIG_MANAGER DA RASPBERRY ==========

class DummyConfigManager:
    def read_variable(self, section, name):
        # Retorna valores padrão simulados
        defaults = {
            'black_max_normal_top': [90, 90, 90],
            'black_max_normal_bottom': [135, 135, 135],
            'black_max_silver_validate_top_off': [50, 50, 50],
            'black_max_silver_validate_bottom_off': [48, 48, 48],
            'black_max_silver_validate_top_on': [60, 60, 60],
            'black_max_silver_validate_bottom_on': [80, 80, 80],
            'black_max_ramp_down_top': [40, 40, 40],
            'black_max_zone': [45, 45, 45],
            'green_min': [40, 50, 45],
            'green_max': [85, 255, 255],
            'green_min_zone': [50, 50, 45],
            'green_max_zone': [110, 255, 255],
            'red_min_1': [0, 100, 90],
            'red_max_1': [10, 255, 255],
            'red_min_2': [170, 100, 100],
            'red_max_2': [180, 255, 255],
            'red_min_1_zone': [0, 100, 90],
            'red_max_1_zone': [10, 255, 255],
            'red_min_2_zone': [170, 100, 100],
            'red_max_2_zone': [180, 255, 255],
        }
        return defaults.get(name, [0, 0, 0])

    def write_variable(self, section, name, value):
        # Apenas mostra que salvaria (não precisa salvar de verdade)
        print(f"[CONFIG] {name} = {value}")

# Criamos uma instância simulada
config_manager = DummyConfigManager()


# ================== SIMULAÇÕES DAS FUNÇÕES DO mp_manager ==================

def empty_time_arr():
    # Cria uma lista vazia para armazenar valores temporais
    return []

def add_time_value(arr, value):
    # Adiciona um novo valor e mantém o histórico pequeno
    arr.append(value)
    if len(arr) > 10:  # mantém só os últimos 10 valores
        arr.pop(0)
    return arr

def get_time_average(arr, weight=0.3):
    # Faz uma média simples dos valores
    if len(arr) == 0:
        return 0
...
# Simula a classe Timer da Raspberry
class DummyTimer:
    def __init__(self):
        pass
    ...
    def get_timer(self, name):
        return True  # sempre retorna "ativo" pra não travar a lógica
...
# Line Cam Loop
########################################################################################################################
# ======= SIMULAÇÃO DE VARIÁVEIS DA RASPBERRY =======
...
objective = Dummy("follow_line")
zone_status = Dummy("begin")
rotation_y = Dummy("none")
line_status = Dummy("line_detected")
obstacle_direction = Dummy("l")

# Valores usados internamente
turn_dir = Dummy("straight")
line_crop = Dummy(0.45)
calibrate_color_status = Dummy("none")
calibration_color = Dummy("l-gl")

# Valores de medição
ramp_ahead = Dummy(False)
red_detected = Dummy(False)
silver_value = Dummy(0)
gap_angle = Dummy(0)
gap_center_x = Dummy(0)
gap_center_y = Dummy(0)
line_detected = Dummy(True)
line_angle = Dummy(0)
line_size = Dummy(0)
line_angle_y = Dummy(0)
black_average = Dummy(0)
min_line_size = Dummy(100)
zone_found_black = Dummy(False)
zone_found_green = Dummy(False)
zone_found_red = Dummy(False)
silver_angle = Dummy(0)
exit_angle = Dummy(0)
line_similarity = Dummy(0)
...
def line_cam_loop():
    ...
    # No PC (OpenCV), não usamos foco manual nem controle de lente
    time.sleep(0.1)

    # no PC, não usamos memória compartilhada
...
if __name__ == "__main__":
    debug_mode = True
    (line_cam_loop()poi[0] + ((0 - poi[1]) / slope, 0), camera_x)
                            else:
                                x = poi[0]
                        ...
                        # --- Aqui: mando comando aos motores, usando a variável calculada ---
                        try:
                            set_motor_speed(line_angle.value, bool(line_detected.value))
                        except Exception:
                            # Se algo der errado com GPIO, para motores em segurança
                            motors_stop()

                    ########################################################################################################################
                    # Zone Loop (mantido para deteção dentro da zona, mas sem 'exit' / saída)
                    ########################################################################################################################
                    elif objective.value == "zone":
                        hsv_image = cv2.cvtColor(cv2_img, cv2.COLOR_BGR2HSV)
                        green_image = cv2.inRange(hsv_image, green_min_zone, green_max_zone)
                        ...
                        # se zone_status está em deposit ou exit (original tinha LED on) - mantive a lógica de remoção de cores
                        if zone_status.value in ["exit", "deposit_red", "deposit_green"]:  # With LEDs on
                            black_image = cv2.inRange(cv2_img, black_min, black_max_normal_bottom)
                            black_image[0:int(camera_y * .4), 0:camera_x] = cv2.inRange(cv2_img, black_min, black_max_normal_top)[0:int(camera_y * .4), 0:camera_x]
                        ...
                        # Note: removi get_exit_angle logic (exit da zona) conforme pedido
                ...
                ########################################################################################################################
                    ...
                    # calibration visual modes (mostrar apenas a cor selecionada)
                    if calibration_color.value == "l-gl":
                        cv2_img = cv2.inRange(cv2.cvtColor(cv2_img.copy(), cv2.COLOR_BGR2HSV), green_min, green_max)
                ...
                # Mostrar imagem (modo debug)
                if debug_mode:
                    cv2.imshow("Line Camera", cv2_img)
                    if cv2.waitKey(1) & 0xFF == ord('q'):
                        break
                else:
                    # modo não-debug: não enviamos para shared_memory (tudo rodando aqui)
                    pass
...
# Executa o loop se estiver em debug_mode True ou se quiser rodar diretamente
if __name__ == "__main__":
    # define debug_mode True se quiser janela de preview e sem shared memory
    debug_mode = True
    # garante que variáveis de configuração existam (mp_manager cria algumas)
    try:
        # Se mp_manager define terminate, etc., ok. Caso contrário, cria placeholders.
        _ = terminate.value
    except Exception:
        # placeholders simples para debug standalone
        class _DummyVar:
            def __init__(self, v=False):
                self.value = v
        terminate = _DummyVar(False)
    # roda
    line_cam_loop()