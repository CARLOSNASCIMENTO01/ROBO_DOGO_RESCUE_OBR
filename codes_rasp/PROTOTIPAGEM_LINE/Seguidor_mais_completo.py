...
GPIO.gpio_claim_input(h, BUTTON)
...
def guinada(graus, velocidade=60, tolerancia=1.9):
    parar()
    ser.write(b"R\n")
    time.sleep(0.05)
    angulo_alvo = graus
    ...
    while True:
        ang_atual    = angulo_z()
        print("Z:", angulo_z())
        erro_guinada = angulo_alvo - ang_atual

        if -tolerancia < erro_guinada < tolerancia:
            parar()
            break

        # velocidade proporcional ao erro, mínimo 45
        vel = np.clip(abs(erro_guinada) * 0.7, 30, velocidade)#0.6
...
def guinadinha(graus, velocidade=60, tolerancia=2.0):
    parar()
    ser.write(b"R\n")
    ...
    while True:
        ...
        # velocidade fixa mínima pra garantir força em ângulos pequenos
        vel = np.clip(abs(erro_guinada) * 0.4, 55, velocidade)
